...
// shadercDebug.exe -f E:\PersonalProjs\bgfx\examples\48-hextile\vs_hextile.sc -o E:\PersonalProjs\bgfx\examples\48-hextile\vs_hextile.bin --type v -i E:\PersonalProjs\bgfx\src --varyingdef E:\PersonalProjs\bgfx\examples\48-hextile\varying.def.sc --platform windows -p vs_5_0 --debug -O 0
...
//uniform float4x4 g_mViewProjection;
uniform float4x4 g_mViewProjection1;
...
VS_OUTPUT main( float3 a_position : POSITION) { VS_OUTPUT _varying_; _varying_.v_position = float3(0.0, 0.0, 0.0);;
{
 float3 vP =  mul(g_mLocToWorld, float4(a_position.xyz, 1.0)).xyz;
_varying_.v_position = vP.xyz;
_varying_.gl_Position = mul(g_mViewProjection, float4(vP.xyz, 1.0));
} return _varying_;
}
