...
cbuffer cbGlobals : register(b0) {
	float4x4 g_mViewToWorld : packoffset(c13.x); // view inverse
	float4x4 g_mScrToView : packoffset(c17.x);
};
...
float3 ProduceHexWeights(float3 W, int2 vertex1, int2 vertex2, int2 vertex3)
{
	...
	int v1 = ((uint)(vertex1.x-vertex1.y))%3;
	if(v1<0) v1+=3;
...
{
	float3 sp = GetTileRate() * Prologue(In);
...
OutputStruct EditedShaderPS(in VS_OUTPUT IN)
{
	OutputStruct OUT = (OutputStruct)0;
	
	OUT.Target0 = GroundExamplePS(IN);
